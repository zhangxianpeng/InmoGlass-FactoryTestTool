...
public class GpsSensorTestActivity extends AbstractTestActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setTitle(R.string.gps_sensor_test_title);
        ...
        if (Math.abs(x) <= Math.abs(y)) {
            ...
            if (mUpTxt.getBackground() != null && mDownTxt.getBackground() != null && mLeftTxt.getBackground() != null && mRightTxt.getBackground() != null) {
                mHandler.postDelayed(new Runnable() {
                    @Override
                    public void run() {
                        Toast.makeText(GpsSensorTestActivity.this, "pass", Toast.LENGTH_SHORT).show();
                        /*SPRD bug 760913:Test can pass/fail must click button*/
                        if (isBoardISharkL210c10()) {
        ...
        mHandler.postDelayed(new Runnable() {
            public void run() {
                if (x != 0 && y != 0 && y != 0) {
                    ...
                    Toast.makeText(GpsSensorTestActivity.this, "fail", Toast.LENGTH_SHORT).show();
                    storeResult(false);
                }
                finish();
            }
